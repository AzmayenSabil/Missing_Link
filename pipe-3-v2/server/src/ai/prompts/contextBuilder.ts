...
import type { Phase1Summary, Phase2Summary, ImpactAnalysis } from "../../contracts/phase3";
...
export function buildCodebaseContext(phase1: Phase1Summary): string {
  ...
  if (convEntries.length > 0) {
    const lines = convEntries.slice(0, 20).map(([k, v]) => `  ${k}: ${JSON.stringify(v)}`);
  ...
  if (ruleList && ruleList.length > 0) {
    sections.push(["## Architectural Rules", ...ruleList.map((r) => `- ${r}`)].join("\n"));
  }
  ...
  if (phase1.allFiles.length > 0) {
    sections.push(["## All File Paths", ...phase1.allFiles.map((f) => `  ${f}`)].join("\n"));
  }
 ...
 */
export function buildImpactContext(phase2: Phase2Summary, impact: ImpactAnalysis): string {
  const sections: string[] = [];
  ...
  if (impact.summary.areas.length > 0) {
    const lines = impact.summary.areas.map(
      (a) => `  ${a.area}: ${(a.confidence * 100).toFixed(0)}% confidence — ${a.rationale.join("; ")}`,
    );
  ...
  const fileLines = sortedFiles.map(
    (f) => `  [${f.score.toFixed(2)}] ${f.role} — ${f.path}: ${f.reasons.join("; ")}`,
  );
  ...
  if (phase2.questions.length > 0 && phase2.answers.length > 0) {
    ...
    for (const q of phase2.questions) {
      ...
      const answerText = a ? (Array.isArray(a.value) ? a.value.join(", ") : a.value) : "Not answered";
  ...
  if (phase2.notes.length > 0) {
    sections.push(["## Analysis Notes", ...phase2.notes.map((n) => `- ${n}`)].join("\n"));
  }
