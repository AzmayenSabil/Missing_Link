...
dotenv.config({ path: path.resolve(__dirname, "../../../.env") });    // pipe-3-v2/.env
dotenv.config({ path: path.resolve(__dirname, "../../../../.env") }); // repo root .env

export const config = {
  openaiApiKey: process.env.OPENAI_API_KEY ?? "",
  port: parseInt(process.env.PORT_PIPE3 ?? process.env.PORT ?? "3003", 10),
  ...
  if (!config.openaiApiKey) {
    console.warn(
      "WARNING: OPENAI_API_KEY is not set. The AI engine will fail.\n" +
        "Create a .env file in pipe-3-v2/ with: OPENAI_API_KEY=sk-...",
    );
  }
}
