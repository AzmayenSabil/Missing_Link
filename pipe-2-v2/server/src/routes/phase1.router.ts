...
import { listPhase1Runs, loadProjectDNA } from "../services/phase1Loader.service";
 ...
 */
phase1Router.get("/runs", (_req, res) => {
 ...
 */
phase1Router.get("/runs/:runId/summary", (req, res) => {
  const runId = req.params.runId;
  const runPath = path.join(config.outDir, "pipe-1", runId);

  if (!fs.existsSync(runPath)) {
    throw new AppError("PHASE1_NOT_FOUND", `Pipe-1 run not found: ${runId}`, 404);
  }
